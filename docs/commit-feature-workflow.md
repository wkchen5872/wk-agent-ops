...
`/commit-feature` 是 `/opsx:apply` 之後、`wt-done` 之前的最後一個 Claude 操作，
...
⑥ 完成摘要
   ...
   │ Docs:     docs/kgi.md（更新 X section） │
   │ Commit:   abc1234 feat: add KGI…        │
   │                                         │
...
### 方案 A：`/commit-feature` 包含 archive

選擇讓 `/commit-feature` 作為唯一的收尾指令，內含完整 archive 流程。
使用者不需要分別執行 `/opsx:archive` 再 `/commit-feature`。
...
都算 archive 完成。`/commit-feature` 在 archive **完全結束後**才繼續，不跳過 sync 流程。
...
```
skills/commit-feature/SKILL.md      ← 專案原始檔（版本控制）
.claude/skills/commit-feature/SKILL.md  ← 安裝位置（Claude Code 讀取）
```
...
- `.claude/skills/commit-feature/SKILL.md` — Skill 定義
