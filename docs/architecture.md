...
scripts/skills/install.sh
      ↓       ↓       ↓       ↓
.claude/ .agents/ .codex/agents/ docs/  ← Installed tooling and managed policy
      ↓
<target-project>/.claude/ .agents/ .codex/agents/ docs/  ← Other projects
...
**Rule:** Modify `template/`, never `.claude/`, `.agents/`, or managed
`.codex/agents/` files directly.
...
│   │   ├── .codex/agents/     ← Project-owned Codex custom-agent TOML
...
├── .codex/agents/              ← Auto-generated project-owned agents only
...
- **template/ → install.sh → .claude/ / .agents/ / .codex/agents/ / managed docs** — one direction only; no reverse; other `.codex/` content remains external
...
| `diff template/common/.codex/agents/... .codex/agents/...` | Confirm Codex custom agents match their templates |
