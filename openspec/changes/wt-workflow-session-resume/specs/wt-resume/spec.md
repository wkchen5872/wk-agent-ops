...
### Requirement: Resume agent session by feature name (old auto-name behavior)
**Reason**: The old behavior of automatically passing `"RD: <name>"` as the session name to `claude --resume` has been replaced. The new default (no --session) shows the interactive list; the auto-name behavior is now only used in `wt-work` resume path.
**Migration**: Use `wt-resume feature123 --session "RD: feature123"` to replicate the old auto-name behavior, or use `wt-work feature123` to continue coding work with auto-resume by name.
