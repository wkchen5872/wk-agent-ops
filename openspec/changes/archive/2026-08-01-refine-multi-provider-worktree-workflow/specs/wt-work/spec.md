...
**Reason**: Gemini CLI is no longer a supported workflow Provider; Antigravity and Codex replace that integration surface.

**Migration**: Use `--agent antigravity` for Google's current CLI or `--agent codex` for Codex. Existing Gemini sessions are not migrated.
