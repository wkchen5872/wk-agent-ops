# entropy-counter-hook

## Purpose

The entropy-counter hook is a PostToolUse hook for AI CLI tools (Claude Code, Gemini CLI) that monitors OpenSpec archive activity. When the number of archived changes since the last entropy-check watermark reaches a configured threshold, it displays a visible banner prompting the user to run an entropy check, and optionally sends a Telegram notification.
